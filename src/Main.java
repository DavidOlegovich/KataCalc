...
public class Main {
    ...
    public static  String[] stringToArr(String str){
        ...
        for (int i=0;i<exchangeArr.length;i++) {
            if(isRomul) {
                String log = Rim.valueOf(exchangeArr[i]).getNum();
                exchangeArr[i] = log;
            }
        }
        return exchangeArr;
    ...
    public static String calculate(String[] arr, String input){
        if(input.contains("/")){
            int op = Calc.DE.action(arr[0],arr[1]);
            return Integer.toString(op);
        } else if (input.contains("*")) {
            int op = Calc.UM.action(arr[0],arr[1]);
            return Integer.toString(op);
        } else if (input.contains("+")) {
            int op = Calc.PL.action(arr[0],arr[1]);
            return Integer.toString(op);
        } else if (input.contains("-")) {
            int op = Calc.MI.action(arr[0],arr[1]);
            return Integer.toString(op);
        }
        return "Oshibka";
    }
